...
public class PopupPanel extends JPanel {
	...
	public PopupPanel(int width, int height, final Saar model) {
		this.model = model;
	...
	private void createMediumOptions() {
		...
		for (int i = 0; i < options.length; i++) {
			switch (i) {
				case 0:
					options[i] = new JRadioButton(Medium.OBJECTIVE_STRING);
					options[i].setActionCommand(Medium.OBJECTIVE_STRING);
					break;
				case 1:
					options[i] = new JRadioButton(Medium.LEADER_AVERAGE_STRING);
					options[i].setActionCommand(Medium.LEADER_AVERAGE_STRING);
					break;
				case 2:
					options[i] = new JRadioButton(Medium.FOLLOWER_STRING);
	...
	private void createCitizenOptions() {
		...
		for (int i = 0; i < options.length; i++) {
			switch (i) {
				case 0:
					options[i] = new JRadioButton(Citizen.DEGROOT_STRING);
					options[i].setActionCommand(Citizen.DEGROOT_STRING);
					if (dataExtract.getOpinionDynamic() == Citizen.DEGROOT)
						options[i].setSelected(true);
					break;
				case 1:
					options[i] = new JRadioButton(Citizen.HEGSELMAN_STRING);
					options[i].setActionCommand(Citizen.HEGSELMAN_STRING);
					if (dataExtract.getOpinionDynamic() == Citizen.HEGSELMAN)
						options[i].setSelected(true);
					break;
				case 2:
					options[i] = new JRadioButton(Citizen.ONGGO_STRING);
					options[i].setActionCommand(Citizen.ONGGO_STRING);
					if (dataExtract.getOpinionDynamic() == Citizen.ONGGO)
						options[i].setSelected(true);
					break;
			}
			options[i].setFont(new Font("Sans Serif", Font.PLAIN, 11));
